...
class BilayerDefaults:
  ...
  layer_separation: float = 0.5
  dipole_strength: float = 6.0
  supercell_shape: str = "sq"
  density_rs: float = 0.25
  seed: int = 42
  ...
  results_dir: str = str(DEBUG_ROOT / "results")
  restore_path: str = ""
  reset_iteration_on_restore: bool = True
  reset_optimizer_on_restore: bool = True
  use_adiabatic_hamiltonian: bool = True
  adiabatic_inter_start: float | str = "auto"
  adiabatic_intra_start: float | str = "auto"
  adiabatic_num_stages: int = 5
  adiabatic_schedule: str = "log"
  adiabatic_intra_final: float = 1.0
  adiabatic_inter_final: float = 1.0
  adiabatic_iterations: int = 1000
  adiabatic_lr_rate: float = 3.0e-3
  adiabatic_auto_scale_factor: float = 0.2
  adiabatic_intra_min_scale: float = 1.0e-2
  adiabatic_inter_min_scale: float = 1.0e-4
...
def _make_training_stages(
    *,
    ...
    fine_iterations: int,
    use_adiabatic_hamiltonian: bool,
    adiabatic_inter_start: float | str,
    adiabatic_intra_start: float | str,
    adiabatic_num_stages: int,
    adiabatic_schedule: str,
    adiabatic_intra_final: float,
    adiabatic_inter_final: float,
    adiabatic_iterations: int,
    adiabatic_lr_rate: float,
    adiabatic_auto_scale_factor: float,
    adiabatic_intra_min_scale: float,
    adiabatic_inter_min_scale: float,
) -> list[dict]:
  ...
  if use_adiabatic_hamiltonian:
    inter_scales = _adiabatic_inter_scales(
        start=adiabatic_inter_start,
        final=adiabatic_inter_final,
    ...
    stages.extend(
        _make_adiabatic_stages(
            ...
            lr_rate=adiabatic_lr_rate,
            schedule=adiabatic_schedule,
            auto_scale_factor=adiabatic_auto_scale_factor,
            intra_min_scale=adiabatic_intra_min_scale,
            inter_min_scale=adiabatic_inter_min_scale))
...
def _adiabatic_inter_scales(
  ...
  if start == "auto":
    return tuple(None for _ in range(num_stages))
...
def _make_adiabatic_stages(
    *,
    ...
    inter_start: float | str,
    intra_start: float | str,
    intra_final: float,
    inter_final: float,
    iterations: int,
    lr_rate: float,
    ...
    intra_min_scale: float,
    inter_min_scale: float,
) -> list[dict]:
  """Returns short KFAC warmup stages for adiabatic Hamiltonian continuation."""
  stages = []
  for index, inter_scale in enumerate(inter_scales):
    inter_name = "auto" if inter_scale is None else f"{inter_scale:g}"
    stage = {
        ...
        "adiabatic_scale_schedule": {
            ...
            "schedule": schedule,
            "intra_start": intra_start,
            "intra_final": intra_final,
            "inter_start": inter_start,
            "inter_final": inter_final,
            "auto_scale_factor": auto_scale_factor,
            "intra_min_scale": intra_min_scale,
            "inter_min_scale": inter_min_scale,
        },
    }
    stages.append(stage)
  return stages
...
def build_config(
    *,
    ...
    use_adiabatic_hamiltonian: bool = DEFAULTS.use_adiabatic_hamiltonian,
    adiabatic_inter_start: float | str = DEFAULTS.adiabatic_inter_start,
    adiabatic_intra_start: float | str = DEFAULTS.adiabatic_intra_start,
    ...
    adiabatic_lr_rate: float = DEFAULTS.adiabatic_lr_rate,
    adiabatic_auto_scale_factor: float = DEFAULTS.adiabatic_auto_scale_factor,
    adiabatic_intra_min_scale: float = DEFAULTS.adiabatic_intra_min_scale,
    adiabatic_inter_min_scale: float = DEFAULTS.adiabatic_inter_min_scale,
  ...
  training_stages = _make_training_stages(
      ...
      use_adiabatic_hamiltonian=use_adiabatic_hamiltonian,
      adiabatic_inter_start=adiabatic_inter_start,
      adiabatic_intra_start=adiabatic_intra_start,
      adiabatic_num_stages=adiabatic_num_stages,
      adiabatic_schedule=adiabatic_schedule,
      adiabatic_intra_final=adiabatic_intra_final,
      adiabatic_inter_final=adiabatic_inter_final,
      adiabatic_iterations=adiabatic_iterations,
      adiabatic_lr_rate=adiabatic_lr_rate,
      adiabatic_auto_scale_factor=adiabatic_auto_scale_factor,
      adiabatic_intra_min_scale=adiabatic_intra_min_scale,
      adiabatic_inter_min_scale=adiabatic_inter_min_scale)
