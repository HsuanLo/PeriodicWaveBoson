...
def make_transformer_block(num_heads: int,
  ...
  def init(key: chex.PRNGKey) -> Mapping[str, jnp.ndarray]:
    ...
    return {
        ...
        "out_w": network_blocks.init_linear_layer(
            out_key, num_heads * value_dim, embed_dim, include_bias=False)["w"],
        "ff1": network_blocks.init_linear_layer(
            ff1_key, embed_dim, ff_dim, include_bias=True),
        "ff2": network_blocks.init_linear_layer(
  ...
  def apply(params: networks.ParamTree, tokens: jnp.ndarray) -> jnp.ndarray:
    ...
    x = tokens + network_blocks.linear_layer(attended, params["out_w"])

    ff_input = ln_apply(params["ff_ln"], x)
    hidden = activation_fct(network_blocks.linear_layer(
        ff_input, **params["ff1"]))
    return x + network_blocks.linear_layer(hidden, **params["ff2"])
...
def make_boson_net(
  ...
  def hidden_apply(params: networks.ParamTree,
    ...
    x = network_blocks.linear_layer(features, **params["embed"])
