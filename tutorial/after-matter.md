...
```text
periodicwave/configs/run_bilayer_bosons.py
```
...
If you change parameters in `run_bilayer_bosons.py`, update the matching
parameters at the top of `evaluate-bilayer.py` so the folder name matches.
...
In `run_bilayer_bosons.py`, the default is:
