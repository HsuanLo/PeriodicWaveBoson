...
`evaluate-energies.py` plots the energy convergence from `train_stats.csv`.
`evaluate-bilayer.py` loads checkpoints and plots:
...
parameters in `run_bilayer_bosons.py` change, those scripts must be updated to
