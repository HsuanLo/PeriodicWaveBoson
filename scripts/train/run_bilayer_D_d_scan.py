...
"""Run one shard of a D,d scan by launching run_bilayer.py."""
...
DEFAULT_MANIFEST = REPO_ROOT / "scan_manifests" / "D_d_manifest.csv"
...
def _read_manifest(path: Path) -> list[dict[str, str]]:
  ...
  missing = [name for name in ("D", "d") if name not in rows[0]]
  if missing:
...
def _row_args(row: dict[str, str]) -> list[str]:
  args = [
      "--dipole-strength", _float_string(row["D"]),
      "--d", _float_string(row["d"]),
  ]
  if row.get("seed"):
