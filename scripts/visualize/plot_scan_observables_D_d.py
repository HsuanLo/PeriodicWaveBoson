...
"""Plot density and structure factor across a D,d bilayer boson scan."""
...
def _deduplicate_run_dirs(run_dirs: list[Path]) -> tuple[list[Path], int]:
  ...
  for run_dir in run_dirs:
    parsed = _parse_run_dir(run_dir)
    key = (float(parsed["dipole"]), float(parsed["d"]))
...
def _plot_density_grid(
  ...
  obs_by_param = {
      (obs.params.dipole_strength, obs.params.d): obs for obs in observables
  }
  plot_d_vals = _plot_d_values(d_values)
  fig_width = max(11.0, 2.10 * len(dipole_values) + 0.75)
  ...
  gridspec = fig.add_gridspec(
      ...
      wspace=0.035,
      hspace=0.055)
  axs = np.empty((len(plot_d_vals), len(dipole_values)), dtype=object)
  ...
  for row, d_value in enumerate(plot_d_vals):
    for col, dipole_value in enumerate(dipole_values):
      ax = axs[row, col]
      obs = obs_by_param.get((dipole_value, d_value))
      if obs is None:
...
def _plot_structure_grid(
  ...
  plot_d_vals = _plot_d_values(d_values)
  fig_width = max(11.0, 2.05 * len(dipole_values) + 0.75)
  ...
  fig.suptitle(
      "Bilayer boson D,d scan: static structure factor", fontsize=18, y=1.025)
  fig.supylabel("d", fontsize=10)
...
def _plot_structure_summary(
  ...
  fig.suptitle(
      "Bilayer boson D,d scan: structure factor summary", fontsize=16, y=1.03)
  panels = [
...
def main() -> None:
  ...
  parser.add_argument("--output-prefix", default="scan_D_d_observables")
  ...
  dipole_values = sorted({obs.params.dipole_strength for obs in observables})
  ...
  same_layer_gr_output = (
      scan_dir / f"{args.output_prefix}_pair_correlation_same_layer_grid.png")
  top_bottom_gr_output = (
      scan_dir / f"{args.output_prefix}_pair_correlation_top_bottom_grid.png")
  _plot_density_grid(
      observables,
      dipole_values,
      d_values,
      density_output,
      mode="overall",
      title="Bilayer boson D,d scan: overall xy density")
  _plot_density_grid(
      observables,
      dipole_values,
      d_values,
      top_density_output,
      mode="top",
      title="Bilayer boson D,d scan: top-layer xy density")
  _plot_density_grid(
      observables,
      dipole_values,
      d_values,
      ...
      mode="bottom",
      title="Bilayer boson D,d scan: bottom-layer xy density")
  _plot_structure_grid(observables, dipole_values, d_values, structure_output)
  ...
  _plot_pair_correlation_grid(
      ...
      same_layer_gr_output,
      mode="same",
      title="Bilayer boson D,d scan: same-layer pair correlation")
  _plot_pair_correlation_grid(
      observables,
      dipole_values,
      d_values,
      top_bottom_gr_output,
      mode="interlayer",
      title="Bilayer boson D,d scan: top-bottom pair correlation")

  print(f"Loaded observables for {len(observables)} runs from {scan_dir}")
  if duplicate_count:
    print(
        f"Collapsed {duplicate_count} duplicate (D, d) runs by checkpoint score")
  if skipped:
